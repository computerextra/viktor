...
export default function Suche() {
  ...
  const [results, setResults] = useState<Array<SearchResult> | undefined>(
    undefined
  );
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    const res = await Search(
      values.Suche.trim().replace("/", "").replace("+49", "0")
    );
    setResults(res);
    setLoading(false);
  };
  ...
  return (
    <>
      <h1 className="text-center">Kunden / Lieferanten Suche</h1>
