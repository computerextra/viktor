import {
  ...
  type MitarbeiterMitAbteilungRes,
...
export default function MitarbeiterTable() {
  const [Mitarbeiter, setMitarbeiter] = useState<
    MitarbeiterMitAbteilungRes[] | undefined
  >(undefined);
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const res = await GetMitarbeitersMitAbteilung();
      setMitarbeiter(res);
      setLoading(false);
    })();
  }, []);
  ...
  const columns: ColumnDef<MitarbeiterMitAbteilungRes>[] = [
    {
      accessorKey: "name",
      header: "Name",
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <p>
            {x.Geburtstag &&
              new Date(x.Geburtstag).toLocaleDateString("de-DE", {
                day: "2-digit",
                month: "2-digit",
              })}
          </p>
  ...
  return (
    <div className="mt-5">
      <Button asChild className="mb-5">
        <Link to="/CMS/Mitarbeiter/Neu">Neuen Mitarbeiter anlegen</Link>
      </Button>
      <p className="mb-2">
        Icon Legende: <br />
      ...
      <CmsTable columns={columns} data={Mitarbeiter ?? []} />
    </div>
  );
}
