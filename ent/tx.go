...
func (tx *Tx) init() {
	tx.Abteilung = NewAbteilungClient(tx.config)
	tx.Angebot = NewAngebotClient(tx.config)
	tx.Ansprechpartner = NewAnsprechpartnerClient(tx.config)
	tx.Aussteller = NewAusstellerClient(tx.config)
	tx.Einkauf = NewEinkaufClient(tx.config)
	...
	tx.Statu = NewStatuClient(tx.config)
	tx.Warenlieferung = NewWarenlieferungClient(tx.config)
}
...
// applies a query, for example: Abteilung.QueryXXX(), the query will be executed
